...
import { MemoryStorage, LocalStorageStorage } from '../../cache/storage';
...
describe('CacheBuilder', () => {
  ...
  describe('method chaining', () => {
    it('should support full method chaining', async () => {
      ...
      const result = await builder.key('chain-test').ttl(30000).persistent().refresh().for(mockFn);
    ...
    it('should allow partial chaining', async () => {
      ...
      const result = await builder.key('partial-test').ttl(15000).for(mockFn);
