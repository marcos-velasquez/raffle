...
describe('RemoveRaffleUseCase', () => {
  ...
  it('should complete with error message on failed remove', async () => {
    ...
    mockRaffleRepositoryService.remove?.mockResolvedValue(E.left(exception));
  ...
  it('should complete with error with has purchases number', async () => {
    ...
    await expect(() => useCase['next'](raffle));

    expect(mockRaffleRepositoryService.remove).not.toHaveBeenCalled();
  });
});
