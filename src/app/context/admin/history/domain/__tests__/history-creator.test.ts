...
describe('HistoryCreator entity', () => {
  ...
  it('should create a History entity with valid file and completed raffle', () => {
    const history = HistoryCreator.create({ file: validFile, raffle: completedRafflePrimitives });
    expect(history).toBeInstanceOf(HistoryCreator);
  ...
  it('should throw if file is not a video', () => {
    ...
    expect(() => HistoryCreator.create({ file: invalidFile, raffle: completedRafflePrimitives })).toThrow(
      'Invalid file format'
    );
  });

  it('should throw if raffle is not completed', () => {
    expect(() => HistoryCreator.create({ file: validFile, raffle: incompleteRafflePrimitives })).toThrow(
